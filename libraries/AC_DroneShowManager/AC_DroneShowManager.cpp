...
#define DEFAULT_LAND_RTL_ALT 1.0f
...
{
    gcs().send_text(MAV_SEVERITY_INFO,"start load show file");
    int fd;
    int retval;
    ...
    if (show_data)
    {
        ...
        if (retval)
        {
            ...
            /// hoppz
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Error while parsing show file :%d", (int) retval);
            /// hoppz
        }
        else
        {
            ...
            if (has_valid_takeoff_time())
            {
                ...
                /// hoppz
                gcs().send_text(MAV_SEVERITY_INFO, "Loaded show: %.1lfs, takeoff at %.1lfs, landing at %.1lfs",
                    _total_duration_sec, _takeoff_time_sec, _landing_time_sec);
                /// hoppz
            ...
            else
            {
                ...
                /// hoppz
                gcs().send_text(MAV_SEVERITY_CRITICAL, "Takeoff or landing time is invalid!");
                gcs().send_text(MAV_SEVERITY_CRITICAL, "Loaded show: %.1lfs, takeoff at %.1lfs, landing at %.1lfs",
                    _total_duration_sec, _takeoff_time_sec, _landing_time_sec);
                /// hoppz
            }
        }
        ...
        if (retval)
        {
            ...
            gcs().send_text(MAV_SEVERITY_CRITICAL, "Error while loading light program: %d", (int) retval);
        }
        else
        {
